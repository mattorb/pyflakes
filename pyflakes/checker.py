...
class Checker(object):
    ...
    def addBinding(self, lineno, value, reportRedef=True):
        ...
        if (isinstance(self.scope.get(value.name), FunctionDefinition)
            ...
            show = True
            if value.source.decorators and self.scope.get(value.name).source.decorators:
                dec1 = self.scope.get(value.name).source.decorators.nodes[0]
                dec2 = value.source.decorators.nodes[0]
                if isinstance(dec1, ast.Getattr) and isinstance(dec2, ast.Getattr):
                    if (dec1.attrname != dec2.attrname and
                        dec1.attrname in ['getproperty', 'setproperty', 'delproperty'] and
                        dec2.attrname in ['getproperty', 'setproperty', 'delproperty']):
                        show = False
                elif isinstance(dec1, ast.Name) and isinstance(dec2, ast.Name):
                    if (dec1.name != dec2.name and
                        dec1.name in ['getproperty', 'setproperty', 'delproperty'] and
                        dec2.name in ['getproperty', 'setproperty', 'delproperty']):
                        show = False
    ...
    def handleDoctests(self, node):
        ...
        for example in examples:
            try:
                ...
                self.report(messages.DoctestSyntaxError, node.lineno + example.lineno + e.lineno)
            else:
                self.linenoOffset += node.lineno + example.lineno
                self.handleChildren(ast)
                self.linenoOffset -= node.lineno + example.lineno
        self.popScope()
